...
class MetricsCallback(Callback):
    ...
    def on_epoch_end(self, state: Runner):
        ...
        if (state.stage_epoch_step + 1) % self.check_interval == 0:
            ...
            self.visualizer.display_current_results(state.stage_epoch_step, sum1/count1,
                                                    name='1mean')
            self.visualizer.display_current_results(state.stage_epoch_step,torch.quantile(q10[q10>0],0.9),
                                                    name='1_0.90_quantile')
            self.visualizer.display_current_results(state.stage_epoch_step, sum0/count0,
                                                    name='0mean')
...
class IteratorCallback(Callback):
    ...
    def on_epoch_end(self, state: Runner):
        ...
        for key in capturing_metrics.keys():# trying to keep best 4
            ...
            if state.stage_epoch_step<=4:continue
            else:
                self.capturing_metrics_list[key].sort()
                self.capturing_metrics_list[key].pop(0)
        if state.stage_epoch_step ==self.last_epoch :
